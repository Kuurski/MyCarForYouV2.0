...
public class MainOverviewController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        vehiculeTable.getSelectionModel().selectedItemProperty().addListener(
                (observable, oldValue, newValue) -> showVehicule(newValue));
	}
	
    private void showVehicule(Vehicule vehicule) {
        if (vehicule != null) {
            // Fill the labels with info from the person object.
        	
        	idVehiculeField.setText(Integer.toString(vehicule.getIdVehicule().get()));
